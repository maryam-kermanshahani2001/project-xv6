...
// number of children created in total(all the children no matter the que number)
#define NUM_CHILDREN 60

int main(int argc, char *argv[])
{
    int result = changePolicy(3);//which is the multi layered queue scheduling policy
    if (result == 0)
    {
        printf(1, "successfullychanged to multi layered queue scheduling policy!\n");
    }
    else
    {
    ...
    for (int i = 0; i < 6; i++)
    {
        for(int j = 0; j<10; j++){
            ...
            {
                child_num = i*10 + j;
                priority = i+1;//in the set priority function the que is settled to
